...
cat ~/.ssh/config | grep -q "~/.ssh/localhost_2201_rustdevuser_ssh_1" || echo "  \033[31m The ~/.ssh/config does not contain the identity ~/.ssh/localhost_2201_rustdevuser_ssh_1. \033[0m"
cat ~/.ssh/config | grep -q "~/.ssh/github_com_git_ssh_1" || echo "  \033[31m The ~/.ssh/config does not contain the identity ~/.ssh/github_com_git_ssh_1. \033[0m"
cat ~/.ssh/config | grep -q "~/.ssh/bestia_dev_luciano_bestia_ssh_1" || echo "  \033[31m The ~/.ssh/config does not contain the identity ~/.ssh/bestia_dev_luciano_bestia_ssh_1. \033[0m"
